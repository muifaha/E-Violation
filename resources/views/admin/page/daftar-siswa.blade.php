    ...
    <div class="card shadow my-2 px-0 mx-1">
        <div class="card-header">
            ...
            <div class="dropdown float-right">
                <button class="btn btn-secondary dropdown-toggle" type="button" data-bs-toggle="dropdown" aria-expanded="false">Filter Kelas</button>
                <ul class="dropdown-menu">
                    @foreach ($kelas as $item)
                    <li>
                        <a class="dropdown-item" href="/daftar-siswa?kelas={{ $item->nama_kelas }}" class="text-decoration-none author">{{ $item->nama_kelas }}</a>
                    </li>
                    @endforeach
                </ul>
            </div>
        ...
        <div class="card-body">
            ...
            <table id="table_data_user" class="table table-bordered display nowrap" cellspacing="0" width="100%">
                ...
                <tbody>
                    ...
                    <tr>
                        <td scope="row">{{ ($siswas->currentpage() - 1) * $siswas->perpage() + $loop->index + 1 }}</td>
                        <td>{{ $siswa->nisn }}</td>
                        <td>{{ $siswa->nama }}</td>
                        <td>{{ $siswa->kelas->nama_kelas }}</td>
                        <td><a href="/guru/histori/{{ $siswa->id }}"><b>{{ $siswa->poin }}</b></a></td>
                        <td data-label="Posisi">
                            <a href="#modalCenter{{ $siswa->id }}" role="button" class="btn btn-sm btn-info" data-bs-toggle="modal">Detail</a>
                            <a href="/pelanggaran/tambah/{{ $siswa->nisn }}" class="btn btn-sm btn-danger ">Tambah</a>
                            <a href="/pelanggaran/kurang/{{ $siswa->nisn }}" class="btn btn-sm btn-success d-inline-flex">Kurang</a>
